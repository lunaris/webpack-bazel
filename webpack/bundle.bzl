...
def _webpack_bundle_impl(ctx):
    ...
    dep_file_paths = _array_literal([d.path for d in ctx.files.deps])
    ...
    ins, _, ms = ctx.resolve_command(tools = [w])
    ...
    ctx.actions.expand_template(
        ...
        substitutions = {
            ...
            "TEMPLATED_dep_file_paths": dep_file_paths,
    ...
    ctx.actions.run(
        executable = w.files_to_run.executable,
        arguments = [
        ...
        inputs = (
            [ctx.file.config, configurator] +
            ctx.files.srcs +
            ctx.files.deps +
            ctx.files.tools +
            ins
        ),
        tools = [w.files_to_run.runfiles_manifest],
        input_manifests = ms,
        outputs = [dist],
    )
...
def _node_module_workspaces_array_literal(xs):
    """Takes a list of `Target`s and returns a string containing a JavaScript
    array literal of workspaces belonging to those `Target`s which potentially
    contain `node_modules` directories."""
    workspaces = {}
    for x in xs:
...
def _tool_paths_array_literal(tools):
    """Takes a list of tool `Label`s and returns a string containing a
    JavaScript array literal of paths exposed by those labels."""
    paths = {}
    for f in tools:
...
def _array_literal(xs):
    """Takes a list of strings and produces a string containing a JavaScript
    array literal of those strings."""
    return "[\"" + "\",\"".join(xs) + "\"]"
